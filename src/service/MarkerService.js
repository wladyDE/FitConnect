...
export const load = (onUpdate) => {
  return onSnapshot(collection(db, "userMarkers"), (querySnapshot) => {
    const activeMarkers = [];

    querySnapshot.docs.forEach(document => {
      const markers = document.data().markers;
      const activeDocumentMarkers = markers.filter(marker => marker.status === 'active');
      activeMarkers.push(...activeDocumentMarkers);
    });

    onUpdate(activeMarkers);
  });
};
